...
type fakeWatchdogStore struct {
	dead []db.Host
	// gatewayStarted is what OldestLiveGatewayStart reports; zero = no live
...
func (f *fakeWatchdogStore) OldestLiveGatewayStart(context.Context, time.Time) (time.Time, bool, error) {
	return f.gatewayStarted, !f.gatewayStarted.IsZero(), nil
}
...
func TestSweepMarksStaleHostsDown(t *testing.T) {
	...
	store := &fakeWatchdogStore{dead: []db.Host{{ID: pinnedID(1)}, {ID: pinnedID(2)}}, gatewayStarted: now.Add(-time.Hour)}
...
func TestSweepStartupGraceSkipsDeathPass(t *testing.T) {
	...
	if len(store.deadCutoffs) != 0 || len(store.downed) != 0 {
		t.Fatalf("death pass ran with no live gateway: cutoffs=%v downed=%v", store.deadCutoffs, store.downed)
	}

	store.gatewayStarted = now.Add(-hostDeadAfter / 2)
	sweep("young gateway")
	if len(store.downed) != 0 {
		t.Fatalf("death pass ran inside the gateway's grace window: downed=%v", store.downed)
	}

	store.gatewayStarted = now.Add(-hostDeadAfter)
	sweep("gateway past grace")
